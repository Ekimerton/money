...
export function getDb() {
  ...
  if (!globalThisAny._db) {
    globalThisAny._db = new Database(dbPath);
    globalThisAny._db.pragma('journal_mode = WAL');

    // Initialize schema
    globalThisAny._db.exec(`
    ...
    try {
      globalThisAny._db.prepare(`ALTER TABLE user_config ADD COLUMN auto_mark_duplicates BOOLEAN DEFAULT FALSE`).run();
    } catch (e) {
      // ignore if column already exists
    }
  }

  return globalThisAny._db;
}
