...
export function Coin3D() {
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
        ...
        // Include vertical movement in the rotation energy
        const move = deltaX + deltaY
        setRotationY((prev) => prev + move * 0.8) // Adjust sensitivity
        gestureVelocity.current = move * 0.8
        lastX.current = e.clientX
        lastY.current = e.clientY
    }
    ...
    const handleWheel = (e: React.WheelEvent) => {
        // Capture vertical wheel scroll and add it to the coin velocity
        const wheelMove = e.deltaY * 0.5
        velocity.current += wheelMove
    }

    return (
        <div className="flex flex-col items-center justify-center">
            <div
                className="relative max-w-128 w-full h-40 pb-4 flex items-center justify-center cursor-grab active:cursor-grabbing touch-none select-none transition-colors"
            ...
            >
                <div
                    className="w-24 h-24 relative origin-center"
                    style={{
                ...
                >
                    ...
                    >
                        <div className="w-[85%] h-[85%] rounded-full border-[2px] border-yellow-500 flex items-center justify-center opacity-80 backdrop-blur-sm shadow-inner">
                            <span className="text-yellow-700 text-4xl font-extrabold font-serif" style={{ textShadow: "1px 1px 0px rgba(255,255,255,0.5)" }}>$</span>
                        </div>
                    </div>
